...
exports.default = ({ logger: _logger, db, filterChats, autoDeleteStatusMessage, }) => {
    const isOlderThan24Hours = (timestamp) => {
        const currentTime = (0, moment_timezone_1.default)(new Date()).tz('Asia/Kolkata');
        const hoursDifference = currentTime.diff((0, moment_timezone_1.default)(timestamp * 1000).tz('Asia/Kolkata'), 'hours');
    ...
    if (autoDeleteStatusMessage) {
        ...
        () => __awaiter(void 0, void 0, void 0, function* () {
            var _a, _b, _c, _d, _e;
            const statusMesasges = yield chats.findOne({ id: 'status@broadcast' }, { projection: { _id: 0 } });
            if (statusMesasges) {
                ...
                if (update.$set.messages.$filter.cond.$not.$or.length > 0) {
                    const updateResult = yield chats.updateOne({ id: 'status@broadcast' }, [update]);
    ...
    const bind = (ev) => {
        ...
        ev.on('messaging-history.set', ({ chats: newChats, contacts: newContacts, messages: newMessages, }) => __awaiter(void 0, void 0, void 0, function* () {
            var _a;
            if (filterChats) {
                newChats = newChats
                    .map((chat) => {
            ...
            if (newChats.length) {
                const chatsAdded = yield chats.bulkWrite(newChats.map((chat) => {
                    return {
                        insertOne: {
                            document: chat,
                        },
            ...
            const oldContacts = yield contacts.bulkWrite(newContacts.map((contact) => {
                return {
                    insertOne: {
                        document: contact,
                    },
            ...
            for (const msg of newMessages) {
                ...
                const chat = yield chats.findOne({ id: jid }, { projection: { _id: 0 } });
                if (chat) {
                    ((_a = chat.messages) === null || _a === void 0 ? void 0 : _a.push({ message: msg })) ||
                        (chat.messages = [{ message: msg }]);
                    yield chats.findOneAndUpdate({ id: jid }, { $set: chat }, { upsert: true });
                }
                else {
        ...
        ev.on('contacts.upsert', (Contacts) => __awaiter(void 0, void 0, void 0, function* () {
            for (const contact of Contacts) {
                yield contacts.updateOne({ id: contact.id }, { $set: contact }, { upsert: true });
            }
        ...
        ev.on('contacts.update', (updates) => __awaiter(void 0, void 0, void 0, function* () {
            for (const update of updates) {
                const contact = yield contacts.findOne({ id: update.id }, { projection: { _id: 0 } });
                if (contact) {
                    Object.assign(contact, update);
                    yield contacts.updateOne({ id: update.id }, { $set: contact }, { upsert: true });
                }
                else {
        ...
        ev.on('chats.upsert', (newChats) => __awaiter(void 0, void 0, void 0, function* () {
            yield chats.bulkWrite(newChats.map((chat) => {
                return {
                    updateOne: {
                        filter: { id: chat.id },
                        update: { $set: chat },
        ...
        ev.on('chats.update', (updates) => __awaiter(void 0, void 0, void 0, function* () {
            // try {
            for (const update of updates) {
                const chat = yield chats.findOneAndUpdate({ id: update.id }, {
                    $set: update,
                }, { upsert: true });
                if (!chat) {
        ...
        ev.on('labels.association', ({ type, association }) => __awaiter(void 0, void 0, void 0, function* () {
            switch (type) {
                case 'add':
                    yield labelAssociations.updateOne({ id: (association === null || association === void 0 ? void 0 : association.chatId) || (association === null || association === void 0 ? void 0 : association.labelId) }, { $set: association }, { upsert: true });
                    break;
                case 'remove':
                    yield labelAssociations.deleteOne({
        ...
        ev.on('chats.delete', (deletions) => __awaiter(void 0, void 0, void 0, function* () {
            for (const item of deletions) {
                yield chats.deleteOne({ id: item });
            }
        }));
        ev.on('messages.upsert', ({ messages: newMessages, type }) => __awaiter(void 0, void 0, void 0, function* () {
            // try {
            switch (type) {
                case 'append':
                case 'notify':
                    for (const msg of newMessages) {
                        ...
                        const chat = yield chats.findOne({ id: jid });
                        if (type === 'notify') {
                            if (!chat) {
                                ...
                                chat.messages
                                    ...
                                    : (chat.messages = [{ message: msg }]);
                                yield chats.updateOne({ id: jid }, { $set: chat }, { upsert: true });
                            }
                        }
                    }
                    break;
    ...
    const fromJSON = (json) => __awaiter(void 0, void 0, void 0, function* () {
        yield chats.updateMany({}, { $set: Object.assign({}, json.chats) }, { upsert: true });
        yield labelAssociations.updateMany({}, { $set: Object.assign({}, (json.labelAssociations || [])) }, { upsert: true });
        const contactsCollection = db.collection('contacts');
        yield contactsCollection.updateMany({}, { $set: Object.assign({}, Object.values(json.contacts)) }, { upsert: true });
   ...
   */
    const getChatById = (jid) => __awaiter(void 0, void 0, void 0, function* () {
        return yield chats.findOne({ id: jid }, { projection: { _id: 0 } });
    });
    return {
        chats,
        contacts,
     ...
     * @returns Label IDs
     **/
        getMessageLabels: (messageId) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        loadMessage: (jid, id) => __awaiter(void 0, void 0, void 0, function* () {
            var _f, _g;
            if (messages[jid]) {
                return messages[jid].get(id);
            }
            const chat = yield chats.findOne({ id: jid }, { projection: { _id: 0 } });
            for (const m of (_f = chat === null || chat === void 0 ? void 0 : chat.messages) !== null && _f !== void 0 ? _f : []) {
                if (((_g = m === null || m === void 0 ? void 0 : m.message) === null || _g === void 0 ? void 0 : _g.key.id) === id) {
                    return m.message;
                }
            }
        }),
        mostRecentMessage: (jid) => __awaiter(void 0, void 0, void 0, function* () {
            var _h, _j, _k;
            const message = ((_h = messages[jid]) === null || _h === void 0 ? void 0 : _h.array.slice(-1)[0]) ||
                ((_k = (_j = (yield chats.findOne({ id: jid }, { projection: { _id: 0 } }))) === null || _j === void 0 ? void 0 : _j.messages) === null || _k === void 0 ? void 0 : _k.slice(-1)[0].message) ||
                undefined;
            return message;
        }),
        fetchImageUrl: (jid, sock) => __awaiter(void 0, void 0, void 0, function* () {
            const contact = yield contacts.findOne({ id: jid }, { projection: { _id: 0 } });
            if (!contact) {
            ...
            if (typeof contact.imgUrl === 'undefined') {
                ...
                yield contacts.updateOne({ id: jid }, { $set: contact }, { upsert: true });
            }
            return contact.imgUrl;
        }),
        getContactInfo: (jid, socket) => __awaiter(void 0, void 0, void 0, function* () {
            const contact = yield contacts.findOne({ id: jid }, { projection: { _id: 0 } });
            if (!contact) {
                return {
                    id: jid,
                    imgUrl: yield (socket === null || socket === void 0 ? void 0 : socket.profilePictureUrl(jid)),
                };
            }
            // fetch image if required
            if (typeof contact.imgUrl === 'undefined' ||
                contact.imgUrl === 'changed') {
                contact.imgUrl = yield (socket === null || socket === void 0 ? void 0 : socket.profilePictureUrl(contact.id, 'image'));
                yield contacts.updateOne({ id: jid }, { $set: Object.assign({}, contact) }, { upsert: true });
            }
            return contact;
        }),
...
//# sourceMappingURL=make-mongo-store.js.map